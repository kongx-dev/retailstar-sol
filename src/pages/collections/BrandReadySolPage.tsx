...
export default function BrandReadySolPage() {
  const { domains, loading } = useDomains({ listed: true });
  const brandConfig = collectionConfigs.find((c) => c.id === 'brand-ready-sol')!;
  ...
  return <CollectionPage config={brandConfig} allDomains={domainRecords} />;
}
