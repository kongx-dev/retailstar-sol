...
export default function MemeSolPage() {
  const { domains, loading } = useDomains({ listed: true });
  const memeConfig = collectionConfigs.find((c) => c.id === 'meme-sol')!;
  ...
  return <CollectionPage config={memeConfig} allDomains={domainRecords} />;
}
