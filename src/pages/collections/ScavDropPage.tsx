...
export default function ScavDropPage() {
  const { domains, loading } = useDomains({ listed: true });
  const scavConfig = collectionConfigs.find((c) => c.id === 'scav-drop')!;
  ...
  return <CollectionPage config={scavConfig} allDomains={domainRecords} />;
}
