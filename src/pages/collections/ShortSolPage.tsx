...
export default function ShortSolPage() {
  const { domains, loading } = useDomains({ listed: true });
  const shortConfig = collectionConfigs.find((c) => c.id === 'short-sol')!;
  ...
  return <CollectionPage config={shortConfig} allDomains={domainRecords} />;
}
